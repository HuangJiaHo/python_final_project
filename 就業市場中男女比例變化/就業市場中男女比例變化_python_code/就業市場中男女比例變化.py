...
#------Read_csv_file-----#
data = pd.read_csv('C:\\Users\\IRVING\\Desktop\\python_final_project\\data\\問題四.csv')
...
try:
    Senior_high_compare['男性佔就業人口比率'] = Senior_high_compare['男性佔就業人口比率'].map(lambda x: str(x)[:-1])
    Senior_high_compare['女性佔就業人口比率'] = Senior_high_compare['女性佔就業人口比率'].map(lambda x: str(x)[:-1])
...
plt.title("就業市場中男女比例變化 1978年 ~ 2019年", fontsize=50)

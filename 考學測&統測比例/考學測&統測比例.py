...
#------Read_csv_file-----#
data = pd.read_csv('C:\\Users\\IRVING\\Desktop\\python_final_project\\data\\考學測、統測比例.csv')
...
try:
    Senior_high_compare['報考學測佔當屆畢業人數比率'] = Senior_high_compare['報考學測佔當屆畢業人數比率'].map(lambda x: str(x)[:-1])
    Senior_high_compare['報考統測佔當屆畢業人數比率'] = Senior_high_compare['報考統測佔當屆畢業人數比率'].map(lambda x: str(x)[:-1])
...
Senior_high_compare.plot.bar(fontsize=15,lw=7)
plt.yticks(np.arange(0, 110, step = 10))
#plt.xticks(np.arange(1994, 2020, step = 1))
plt.title("畢業生報考學測 & 統測比例 1994年 ~ 2019年", fontsize=50)
